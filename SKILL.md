...
|---|---|
| 中文文章/文案/博客/公众号 | 1) `humanizer-zh`（主改写） 2) `stop-slop`（质检）；有作者旧文样本时先跑 `nuwa-skill` 蒸文风 |
...
2. **一次重写**：主技能按处理流程整段重写，围绕段落主旨重述，而不是对标记过的短语逐个打补丁。若 nuwa-skill 文风档案存在，作为"作者样本"注入本次重写——规则与文风同一刀，避免两个改写器串行互相拆台
...
2. **作者样本优先**：nuwa 文风档案或用户提供的过往文章样本，其习惯（句长、用词、连词、破折号频率）覆盖 stop-slop 的绝对禁令
